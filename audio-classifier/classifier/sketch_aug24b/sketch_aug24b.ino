 ...
 **
 ** Cortex M0+ has no hardware floating point support, therefore DSP
...
/* Includes ---------------------------------------------------------------- */
#include <Rpi_Pico_Audio_inferencing.h>
 ...
 */
static void adc_data_ready_inference_callback(void)
{
    // Read the value from the ADC (pin 26)
    int adc_value = analogRead(26);

    if ((inference.buf_ready == 0) && (record_ready == true)) {
        inference.buffers[inference.buf_select][inference.buf_count++] = adc_value;
...
/**
 * @brief      Init inferencing struct and setup/start ADC
 *
...
static bool microphone_inference_start(uint32_t n_samples)
{
    ...
    analogRead(26); // Just to ensure the pin is active
...
static bool microphone_inference_record(void)
{
    ...
    while (inference.buf_ready == 0) {
        adc_data_ready_inference_callback();  // Read data from ADC
...
/**
 * @brief      Stop ADC and release buffers
 */
